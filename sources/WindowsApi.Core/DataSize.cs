...
namespace DustInTheWind.WindowsApi.Core
{
    ...
    public struct DataSize : IEquatable<DataSize>
    {
        ...
        public static DataSize OneByte { get; } = new DataSize(1);
        public static DataSize OneKilobyte { get; } = new DataSize(1024);
        ...
        public override string ToString()
        {
            ...
            if (tempSize >= OneGigabyte)
            {
                ...
                if (tempSize < 10)
                    return string.Format("{0:0.0} GB", tempSize);

                return string.Format("{0:N0} GB", tempSize);
            }

            if (tempSize >= OneMegabyte)
            {
                tempSize /= OneMegabyte;
                return string.Format("{0:N0} MB", tempSize);
            }

            if (tempSize >= OneKilobyte)
            {
                tempSize /= OneKilobyte;
                return string.Format("{0:N0} KB", tempSize);
            }

            return string.Format("{0:N0} B", tempSize);
        }
        ...
        {
            string shortValue = ToString();
            string longValue = ToString(DataSizeUnit.Byte);

            return string.Format("{0} ({1})", shortValue, longValue);
        }

        public string ToString(DataSizeFormatOptions formatOptions)
        {
            object[] values = formatOptions.Values
                .Select(To)
                .Cast<object>()
                .ToArray();

            return string.Format(formatOptions.Template, values);
        }
